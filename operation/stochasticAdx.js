...
const adxAlgorithms = [
  {
    open: (p, c) => adxPlusDiAbove(p, c),
    algo: 'plusDIAbove'
  },
  {
    open: (p, c) => adxPlusDiUnder(p, c),
    algo: 'plusDIUnder'
  },
  {
...
(async () => {
  ...
  for (let x = 0; x < adxAlgorithms.length; x ++) {
    ...
    for (let y = 0; y < stochasticAlgorithms.length; y ++) {
      ...
      await performStochasticAlgorithm(periods, adxAlgo, stochasticAlgo, daysOfPeriods)
    }
  }
...
const performStochasticAlgorithm = (periods, adxAlgo, stochasticAlgo, daysOfPeriods) => {

const {
  candleStats,
  isTimesBigger,
} = require("@/operation/candlePatterns/service");
...
exports.wmaConjoinedStochastic = (prior, current) => {
  ...
  return {
    openConditions:
      wmaOver(current, 10, 50) && stochasticCrossedOver(prior, current, 50),
    closeConditions:
      wmaUnder(current, 10, 50) || stochasticCrossedUnder(prior, current, 70),
...
exports.adxCrossover = (prior, current) =>
  prior.adx.plusDi <= prior.adx.minusDi &&
  current.adx.plusDi > current.adx.minusDi;

exports.adxCrossunder = (prior, current) =>
  prior.adx.minusDi >= prior.adx.plusDi &&
  current.adx.minusDi < current.adx.plusDi;

exports.adxPlusDiUnder = (prior, current) =>
  current.adx.plusDi <= current.adx.minusDi;
...
exports.adxAboveThreshold = (prior, current, threshold) =>
  current.adx.adx >= threshold;

exports.adxBelowThreshold = (prior, current, threshold) =>
  current.adx.adx <= threshold;

exports.adxPlusDiAboveThreshold = (prior, current, threshold) =>
  current.adx.plusDi > threshold;
...
exports.adxBelowDiBelowThreshold = (current, threshold) =>
  current.adx.minusDi > threshold;

exports.adxPlusDiBelowThreshold = (prior, current, threshold) =>
  current.adx.plusDi < threshold;

exports.macdCrossedOver = (prior, current) => {
  return (
    prior.macd.macdLine < prior.macd.macdLag &&
...
exports.macdUnder = (prior, current) =>
  current.macd.macdLine <= current.macd.macdLag;
...
const candleBodyLength = (candle) =>
  parseFloat(candle.o) - parseFloat(candle.l);
...
exports.progressiveTrendUp = (current, haTrendGroups, haCandles) => {
  const progressiveTrendUp = getProgressiveCandleTrend(
    current,
    haTrendGroups,
    haCandles
  );
...
exports.periodLow = (current, candles, length) => {
  ...
  const priorCandleLow = priorCandles.reduce((a, b) =>
    a.close < b.close ? a : b
  );
...
exports.periodHigh = (current, candles, length) => {
  ...
  const priorCandles = relevantCandles.splice(0, length - 1);
  const priorCandleHigh = priorCandles.reduce((a, b) =>
    a.close > b.close ? a : b
  );
